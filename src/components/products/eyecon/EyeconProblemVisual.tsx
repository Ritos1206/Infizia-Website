...
/**
 * EyeCON-specific problem section.
 *
 * Visualizes the wedge: five disconnected legacy tools collapse into a
 * single EyeCON tile in the center, with animated dashed lines flowing
 * inward to suggest consolidation.
 */
export function EyeconProblemVisual({
  kicker,
  title,
  body,
}: {
...
/* -------------------------------------------------------------------------- */
/*  Tools-convergence visual: 5 legacy tool tiles → 1 EyeCON tile               */
/* -------------------------------------------------------------------------- */

const TOOLS = [
  { Icon: Database, label: "CRM", subtitle: "Salesforce / HubSpot" },
  { Icon: Phone, label: "Dialer", subtitle: "Aircall / CallHippo" },
  { Icon: Search, label: "Research", subtitle: "Apollo / Sales Nav" },
  { Icon: FileText, label: "Notes", subtitle: "Notion / Sticky notes" },
  { Icon: Mic, label: "Recorder", subtitle: "Gong / Fireflies" },
];

function ToolsConvergence() {
  return (
    <div className="relative">
      {/* Subtle radial glow behind the EyeCON tile */}
      <div className="pointer-events-none absolute left-1/2 top-1/2 -z-10 h-72 w-72 -translate-x-1/2 -translate-y-1/2 rounded-full bg-brand-teal/[0.08] blur-3xl" />
      ...
      <div className="grid grid-cols-2 items-center gap-6 lg:grid-cols-12 lg:gap-4">
        ...
        {/* Center: EyeCON tile + connector lines */}
      ...
      <div className="mt-8 flex flex-wrap items-center justify-center gap-x-4 gap-y-2 text-xs text-text-faint">
        ...
        <span className="font-mono uppercase tracking-[0.18em] text-brand-teal">
          → 1 surface
        </span>
        <span className="h-px w-8 bg-white/10" />
        <span className="font-mono uppercase tracking-[0.18em] text-text-muted">
...
function IconCenterTile() {
  return (
    <motion.div
      ...
      className="relative z-10 flex h-32 w-32 flex-col items-center justify-center rounded-2xl border border-brand-teal/30 bg-gradient-to-b from-brand-teal/[0.15] to-brand-teal/[0.05] backdrop-blur shadow-[0_0_60px_-10px_rgba(94,234,212,0.5)] sm:h-40 sm:w-40"
    >
      {/* Pulsing aura */}
      <motion.div
        className="absolute inset-0 rounded-2xl border border-brand-teal/30"
        animate={{ scale: [1, 1.08, 1], opacity: [0.6, 0.2, 0.6] }}
        transition={{ duration: 3, repeat: Infinity, ease: "easeInOut" }}
      />

      <Sparkles className="h-5 w-5 text-brand-teal sm:h-6 sm:w-6" />
      <p className="mt-2 font-display text-xl font-semibold text-white sm:text-2xl">
        EyeCON
      </p>
      <p className="font-mono text-[9px] uppercase tracking-[0.18em] text-brand-teal sm:text-[10px]">
        One surface
      </p>
    </motion.div>
  );
...
/**
 * Animated dashed lines flowing from the four corners toward the center.
...
function ConnectorLines() {
  return (
    ...
    >
      <defs>
        <linearGradient id="iconLineL" x1="0" y1="0" x2="1" y2="0">
          <stop offset="0%" stopColor="rgba(94,234,212,0)" />
          <stop offset="100%" stopColor="rgba(94,234,212,0.6)" />
        </linearGradient>
        <linearGradient id="iconLineR" x1="1" y1="0" x2="0" y2="0">
          <stop offset="0%" stopColor="rgba(94,234,212,0)" />
          <stop offset="100%" stopColor="rgba(94,234,212,0.6)" />
        </linearGradient>
      </defs>
