...
import type { TechPattern } from "@/content/technology/types";

/**
 * Tech pillar patterns — 4 architectural patterns / approaches we
 * apply when engaging on this pillar. Mirrors `ServiceEngagement`
 * shape (4-card row) but reframes label as "pattern" + adds a tag pill.
 */
export function TechPatterns({
  kicker,
  title,
  lede,
  items,
          ...
          {items.map((m, i) => {
            ...
            return (
              ...
              >
                ...
                <span
                  className={cn(
                    "mt-2 inline-flex items-center rounded-full border px-2 py-0.5 font-mono text-[10px] uppercase tracking-[0.16em]",
                    a.border,
                    a.bgSoft,
                    a.text,
                  )}
